<template>
    <div class="app-retrieve-pwd">
        ...
        <ul class="user-info">
            <li v-if="comefrom === 1" class="input-item">
                <h6>登录账号: {{ userName }}</h6>
            </li>
            <li v-else class="input-item">
                <div class="title">
                    <img src="@/assets/images/icon-login-2.png" class="icon">
                    <span>登录账号</span>
                </div>
                <p :class="['content', userNameError ? 'is-error' : '']">
                    <input v-model.trim="userName" type="text" placeholder="请输入登录账号" @blur="checkAccount" @input="updateName">
            ...
            <li class="input-item">
                ...
                <p class="content">
                    <input v-model.trim="phone" disabled class="flex-none" type="number" placeholder="请输入手机号码">
                    <!-- <img v-show="phone" src="@/assets/images/empty-word.png" @click="phone = ''"> -->
                    <i class="line" />
                    <span class="line-text" @click="changePhone">手机号变更?</span>
                </p>
            </li>
            <li class="input-item">
                <div class="title">
        ...
        <van-dialog v-model="show" :show-confirm-button="false">
            <div class="dialog-box">
                ...
                <p v-if="auditStatus === 0">您的资料正在审核中，我们将在48小时内反馈结果，请耐心等待~</p>
                <p v-else-if="auditStatus === 1">恭喜您！已通过审核，手机号码已变更点击下方按钮去登录吧！</p>
                <p v-else-if="auditStatus === 3">此密码很容易被猜到<br>您的密码将用于登录恒昌健康APP</p>
                <p v-else-if="auditStatus === 4">手机号变更只修改【用户验证手机号】<br>并不修改账号和收货联系人手机号</p>
                <p v-else v-html="reason">{{ reason }}</p>
            </div>
            <div class="dialog-footer">
                <div v-if="cancelStr" class="btn cancel-btn" @click="cancelFn">{{ cancelStr }}</div>
                <div class="btn confirm-btn" @click="confirmFn">{{ confirmStr }}</div>
            </div>
        </van-dialog>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            ossUrl: '',
            account: '',
            from: this.$route.query.title === '修改密码' ? 1 : 0
        };
    },
    computed: {
        title() {
    ...
    created() {
        ...
        if (this.userName) this.checkAccount();
    },
    methods: {
        // 获取手机号码
        checkAccount() {
            ...
            if (this.userName) {
                ...
                this.userNameError = '登录账号不能为空或空字符';
            }
        },
        // 修改名称
        updateName() {
        ...
        changePhone() {
            if (this.userName && this.phone){
                Api.progressQuery(this.userName).then(({ data: { data }}) => {
                    if (!data) {
                        this.show = true;
                        this.auditStatus = 4;
                        ...
                        this.reason = data.reason + '，未通过审核<br>请重新申诉' || '';
                    }
                })
            } else {
                Toast('请先输入登录账号');
        ...
        updatePwd(sign) {
            ...
            if (sign === 1) params.sign = 1;
            Api.forgetEditPwd(params).then(({ data }) => {
                if (data.code === 100032) {
                    this.auditStatus = 3;
                    this.show = true;
                } else if (data.code === 0) {
